...
class ReportController extends Controller
{
    ...
    private function jml($data)
    {
		$part_master_id = [];
		foreach ($data as $value) {
			$company = PartBinLocation::select('tbl_company.id as tbl_company_id', 'company', 'uom_type')
		        ->join('tbl_company', 'tbl_company.id', '=', 'part_bin_location.tbl_company_id')
		        ->where('part_master_id', Hashids::decode($value->part_master_id)[0])
		        ->distinct()->get();
    ...
    private function fixedReport($key, $current, $generate)
	{
		...
		$indeks = 1;
		foreach ($data as $value) {
			$part_master_id = Hashids::decode($value->part_master_id)[0];
		    ...
		    if(count($company)>0){
		    	foreach ($company as $companyValue) {
		    		...
		    		$tbl_company_id = Hashids::decode($companyValue->tbl_company_id)[0];
					...
					if($companyValue->uom_type == 2){
						$unit_issue = $new_master->unit2;
					}elseif($companyValue->uom_type == 3){
						$unit_issue = $new_master->unit3;
					}elseif($companyValue->uom_type == 4){
					...
					$html .= '<div style="position: relative; height: 1000px;"><h2><center>LAPORAN HARIAN</center></h2> 
						...
						<h1><center>'.$companyValue->company.'</center></h1>
						<br>
	...
	public function report($keys)
	{
		...
		if($type == 1){
			return $this->fixedReport($key, $current, $generate);
		}else{
			return $this->fullReport($key, $current, $generate);
		}
	}
